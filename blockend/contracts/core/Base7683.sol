...
import {GaslessCrossChainOrder, OnchainCrossChainOrder, ResolvedCrossChainOrder, Output, FillInstruction, IOriginSettler, IDestinationSettler} from "./ERC7683/IERC7683.sol";
...
abstract contract Base7683 is IOriginSettler, IDestinationSettler {
    ...
    bytes32 public constant RESOLVED_CROSS_CHAIN_ORDER_TYPEHASH =
        keccak256(
    ...
    function settle(bytes32[] calldata _orderIds) external payable {
        ...
        for (uint256 i = 0; i < _orderIds.length; i += 1) {
            ...
            if (orderStatus[_orderIds[i]] != FILLED)
    ...
    function _permitTransferFrom(
        ...
        ISignatureTransfer.TokenPermissions[]
            memory permitted = new ISignatureTransfer.TokenPermissions[](
                _resolvedOrder.minReceived.length
            );
        ...
        for (uint256 i = 0; i < _resolvedOrder.minReceived.length; i++) {
            permitted[i] = ISignatureTransfer.TokenPermissions({
                token: TypeCasts.bytes32ToAddress(
                    _resolvedOrder.minReceived[i].token
                ),
