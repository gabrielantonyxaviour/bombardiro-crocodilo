 ...
 * layer.
 */
abstract contract World7683 is Base7683 {
    // ============ Libraries ============
    ...
    /// @notice Status constant indicating that an order has been settled in World Chain.
    bytes32 public constant SETTLED_WORLD = "SETTLED_WORLD";
    /// @notice Status constant indicating that an order has been refunded in World Chain.
    ...
    /**
     * @notice Emitted when an order is settled in World Chain.
     * @param orderId The ID of the settled order.
     */
    ...
    /**
     * @notice Emitted when an order is refunded in World Chain.
     * @param orderId The ID of the refunded order.
     */
    event RefundedWorld(bytes32 orderId);

    /**
     * @notice Emitted when an order is failed.
     * @param orderId The ID of the failed order.
     * @param receiver The address of the order's input token receiver.
     */
    event Failed(bytes32 orderId, address receiver);

    // ============ Errors ============

    error InvalidOrderType(bytes32 orderType);
    error InvalidOriginDomain(uint32 originDomain);
    error InvalidOnchainOrderWorldSettler();
    ...
    function openFor(
        ...
        if (_order.worldSettler != address(this))
            revert InvalidGaslessOrderSettler();
    ...
    function open(
        ...
        if (_order.worldSettler != address(this))
            revert InvalidOnchainOrderWorldSettler();
        (
            ResolvedCrossChainOrder memory resolvedOrder,
            bytes32 orderId,
            uint256 nonce
    ...
    function _settleOrders(
        ...
        for (uint256 i = 0; i < _orderIds.length; i++) {
            orderStatus[_orderIds[i]] = SETTLED_WORLD;
    ...
    function _refundOrders(
        ...
        for (uint256 i = 0; i < _orderIds.length; i++) {
            orderStatus[_orderIds[i]] = REFUNDED_WORLD;
    ...
    function _handleSettleOrder(
        ...
        if (orderData.originDomain == worldDomainId) {
            address inputToken = TypeCasts.bytes32ToAddress(
    ...
    function _checkOrderEligibility(
        ...
        if (orderData.originDomain == worldDomainId) {
            ...
            if (
                orderData.originDomain != _messageOrigin ||
                orderData.originSettler != _messageSender
            ) return (false, orderData);
        }
